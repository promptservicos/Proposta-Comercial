...
// Carregar tema salvo
const savedTheme = localStorage.getItem('theme_prompt');
if (savedTheme === 'dark') {
    ...
    if (!savedTheme) localStorage.setItem('theme_prompt', 'light');
}
...
themeToggle.addEventListener('click', () => {
    ...
    if (isDark) {
        themeIcon.classList.remove('bx-moon');
        themeIcon.classList.add('bx-sun');
        localStorage.setItem('theme_prompt', 'dark');
    } else {
        themeIcon.classList.remove('bx-sun');
        themeIcon.classList.add('bx-moon');
        localStorage.setItem('theme_prompt', 'light');
    }
});
...
function setLoading(isLoading) {
    if (loginBtn) {
        if (isLoading) {
            ...
            loginBtn.innerHTML = '<span>Entrar</span><i class="bx bx-log-in"></i>';
        }
    }
}
